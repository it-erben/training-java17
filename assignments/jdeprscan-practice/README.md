...
Dieses Modul enthält absichtlich veraltete API-Verwendungen, damit du den JDK-Scanner `jdeprscan` ausprobieren kannst. Die Klassen unter `tech.erben.java17.jdeprscan` nutzen unter anderem `SecurityManager`, `Thread.stop/suspend/resume`, `Runtime.runFinalizersOnExit`, `Observable/Observer` und `java.security.acl.Acl`.
